...
std::string FFT::get_serial_result(std::string s1, std::string s2) {
...
std::string FFT::get_parallel_result(std::string s1, std::string s2) {
	FFT::set_vector(s1, s2);
	vector<int> result = parallel_multiply(v1, v2);
	return parallel_get_result_from_vector(result);
}

void FFT::set_vector(std::string s1, std::string s2) {
	v1 = string_to_vi(s1);
	v2 = string_to_vi(s2);
...
std::string FFT::seial_get_result_from_vector(vector<int> &ret) {
	int i = 0;
	string s = "";
	while (i < ret.size()) {
		if (ret[i] >= 10) {
...
std::string FFT::parallel_get_result_from_vector(vector<int> &ret) {
	int i = 0;
	string s = "";
	while (i < ret.size()) {
		if (ret[i] >= 10) {
